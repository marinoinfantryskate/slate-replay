  ...
  return (
    <div className="px-4 pt-4">
      <div className="flex items-center justify-between text-xs text-muted">
        <span>
          {game.when} · {game.dayNight === "day" ? "Day" : "Night"}
          {game.divisional && " · Division game"}
        </span>
      </div>
...
function TeamName({ abbr, name, right }: { abbr: string; name: string; right?: boolean }) {
  return (
    <div className={`min-w-0 flex-1 ${right ? "text-right" : ""}`}>
      <div className="text-2xl font-bold tracking-tight">{abbr}</div>
...
export default function GameCard({ game, disabled, selection, onPick }: Props) {
  const { ml, total } = game.odds;
  return (
    <article className="overflow-hidden rounded-2xl border border-line bg-panel">
      <MatchupHeader game={game} />

      <div className="mt-4 space-y-2 border-t border-line bg-panel-2/60 px-4 py-3">
        <MarketRow label="Moneyline">
          <PickButton
            active={selection("moneyline") === "away"}
            disabled={disabled}
            onClick={() => onPick("moneyline", "away")}
            top={game.away.abbr}
            odds={ml.away}
          />
          <PickButton
            active={selection("moneyline") === "home"}
            disabled={disabled}
            onClick={() => onPick("moneyline", "home")}
            top={game.home.abbr}
            odds={ml.home}
          />
        </MarketRow>
        <MarketRow label={`Total ${total.line}`}>
          <PickButton
            active={selection("total") === "over"}
            disabled={disabled}
            onClick={() => onPick("total", "over")}
            top={`Over ${total.line}`}
            odds={total.over}
          />
          <PickButton
            active={selection("total") === "under"}
            disabled={disabled}
            onClick={() => onPick("total", "under")}
            top={`Under ${total.line}`}
            odds={total.under}
          />
        </MarketRow>
      </div>

      <Tier1 away={game.pre.away} home={game.pre.home} />
      <p className="px-4 pb-3 text-[11px] text-muted">Closing line · {game.odds.source}</p>
    </article>
  );
}
...
function PickButton(props: { active: boolean; disabled: boolean; onClick: () => void; top: string; odds: number }) {
  return (
    <button
      type="button"
      ...
      className={`rounded-lg border px-2 py-2 text-center transition disabled:cursor-not-allowed ${
        props.active
          ? "border-accent bg-accent text-accent-ink"
          : "border-line bg-panel hover:border-muted disabled:opacity-60"
      }`}
    >
      <div className="text-sm font-semibold">{props.top}</div>
...
function Tier1({ away, home }: { away: TeamPre; home: TeamPre }) {
...
function StarterBox({ s, right }: { s: Starter; right?: boolean }) {
  ...
  return (
    <div className={`rounded-lg bg-panel-2 p-2.5 ${right ? "text-right" : ""}`}>
      ...
      {s.era == null ? (
        ...
        <>
          <div className={`mt-2 grid grid-cols-3 gap-1 ${right ? "text-right" : ""}`}>
          ...
          {s.last3.length > 0 && (
            <div className="mt-2 border-t border-line pt-2">
              ...
              <div className={`mt-1 flex flex-wrap gap-1 ${right ? "justify-end" : ""}`}>
                {s.last3.map((x, i) => (
                  <span key={i} className="rounded bg-panel px-1.5 py-0.5 font-mono text-[11px] tnum">
                    {x.ip}/{x.er}/{x.k}
                  </span>
                ))}
              </div>
